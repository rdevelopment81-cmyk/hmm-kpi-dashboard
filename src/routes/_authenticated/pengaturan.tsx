...
function SettingsPage() {
  ...
  const { data: settings } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("kpi_settings").select("*").eq("id", 1).maybeSingle()).data,
  });
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["kpi_settings"] });
    },
    onError: (e: Error) => toast.error(e.message),
  });

  if (!user?.roles.includes("hr_admin"))
    return <p className="text-muted-foreground">Hanya HR Admin.</p>;
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Atur bobot kontribusi kehadiran dan jobdesk terhadap skor akhir.
        </p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Bobot perhitungan</CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="rounded-md bg-muted p-4 text-sm">
            ...
            <code className="text-xs">
              KPI = kehadiran% × {(attendance / 100).toFixed(2)} + jobdesk% ×{" "}
              {((100 - attendance) / 100).toFixed(2)}
            </code>
          </div>
