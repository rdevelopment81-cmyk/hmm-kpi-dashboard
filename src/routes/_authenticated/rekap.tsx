...
function RekapPage() {
  ...
  const { data: profiles } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("profiles").select("*, divisions(name,code)").order("full_name")).data ??
      [],
  });
  ...
  const rows = useMemo(() => {
    const list = (profiles ?? []).filter(
      (p: any) => divisionId === "all" || p.division_id === divisionId,
    );
    return list
      .map((p: any) => ({ ...p, kpi: kpiMap?.[p.id] ?? {} }))
  ...
  function exportCSV() {
    ...
    a.href = url;
    a.download = `rekap-kpi-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Urut dari skor tertinggi. Dapat diekspor ke CSV.
          </p>
        </div>
              ...
              <tbody>
                {rows.map((r: any, i: number) => (
                  <tr key={r.id} className="border-t border-border hover:bg-muted/30">
                    ...
                    <td className="p-3">
                      <Badge variant="outline">{r.divisions?.code ?? "-"}</Badge>
                    </td>
